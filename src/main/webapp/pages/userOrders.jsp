...
<title>用戶訂單．Flower Forever</title>
<base href="<%=basePath%>">
<!-- Bootstrap -->
					...
					<c:forEach items="${requestScope.myOrders}" var="order">
						<tr>
							...
							<td><a class="changeOrderStatus" id="changeOrderStatus"
								href="<%=path %>:8080/MyFlower2.0/orderServlet?action=changeOrderStatus2&orderNo=${order.orderNo}">更新為
									已簽收</a></td>
							<!-- 刪除鍵做防呆    &   此處id名可自定義，不用對應到哪個 -->
							<td><a class="details" id="details"
								href="<%=path%>:8080/MyFlower2.0/orderServlet?action=showOrderDetails2&orderNo=${order.orderNo}">訂單詳情</a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
