...
<script type="text/javascript">
	$(function() {
		...
		if(status == 'admin'){
			$("tbody td:gt(8)").attr('disabled', true);
		} */
	...
	<div class="content">
		...
		<div class="content-right">
			...
			<div class="header_insert">
				<div id="title">會員管理</div>
				<div id="add">
					<a href="<%=path %>:8080/MyFlower2.0/pages/member_insert.jsp">新增會員</a>
				</div>
			</div>
		<hr>
			<!--  <div class="list">  -->
					...
					<tbody>
						<c:forEach items="${requestScope.members}" var="member">
							<tr>
								...
								<td>${member.phone}</td>
								<td>${member.status}</td>
								<td><a href="<%=path%>:8080/MyFlower2.0/manager/UserServlet?action=getMember&account=${member.account}">修改</a></td>
								<!-- 刪除鍵做防呆    &   此處id名可自定義，不用對應到哪個 -->
								<td><a class="delete" id="delete" href="<%=path%>:8080/MyFlower2.0/manager/UserServlet?action=delete&id=${member.id}">刪除</a></td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
