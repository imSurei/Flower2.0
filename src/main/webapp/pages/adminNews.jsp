	...
	<div class="content">
		...
		<div class="content-right">
			...
			<div class="header_insert">
				<div id="title">消息管理</div>
				<div id="add">
					<a href="<%=path %>:8080/MyFlower2.0/pages/news_insert.jsp">新增消息</a>
				</div>
			</div>
		<hr>
			<!--  <div class="list">  -->
					...
					<tbody>
						<c:forEach items="${requestScope.news}" var="news">
							<tr>
								...
								<td>${news.title}</td>
								<td>${news.content}</td>
								<td><a href="<%=path%>:8080/MyFlower2.0/manager/NewsServlet?action=getNews&id=${news.id}">修改</a></td>
								<!-- 刪除鍵做防呆    &   此處id名可自定義，不用對應到哪個 -->
								<td><a class="delete" id="delete" href="<%=path%>:8080/MyFlower2.0/manager/NewsServlet?action=delete&id=${news.id}">刪除</a></td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
